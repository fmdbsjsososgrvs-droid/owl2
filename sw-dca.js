// DCA CALCULATOR 오프라인 캐시
// 한 번 온라인으로 열면 이 페이지를 저장해두고, 이후엔 네트워크 없이도 그대로 열립니다.
// (가격/차트 데이터는 실시간 API 호출이라 오프라인일 땐 못 불러오지만, 페이지 자체는 열립니다.)
// 주의: 이 파일의 scope는 등록부(dca.html)에서 '/dca.html'로 좁혀서 등록하지만,
// 구형 브라우저 대비로 fetch 핸들러에서도 이 자산 하나만 명시적으로 가로챈다.
const CACHE_NAME = "dca-v1";
const ASSET = "/dca.html";
